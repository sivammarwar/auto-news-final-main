  ...
  return (
    ...
    >
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <Link href={`/category/${article.category}`} className="font-mono text-[10px] uppercase tracking-[0.15em] text-primary font-bold hover:underline">
          {article.category}
        </Link>
        <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-muted-foreground shrink-0">{timeAgo}</span>
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-border">
        <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-muted-foreground">{article.source_name}</span>
      </div>
    </motion.article>
  );
};
