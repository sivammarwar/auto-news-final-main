    ...
    <footer className="bg-background border-t border-border pt-12 pb-8 px-4 sm:px-6">
      <div className="max-w-screen-xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-10">
          ...
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground mb-3">Sections</p>
            <div className="flex flex-wrap gap-x-5 gap-y-2">
              {FOOTER_CATEGORIES.map(cat => (
                <Link key={cat.path} href={cat.path} className="font-mono text-[11px] uppercase tracking-[0.12em] text-muted-foreground hover:text-primary transition-colors">
                  {cat.label}
                </Link>
              ))}
            </div>
